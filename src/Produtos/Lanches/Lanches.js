...
import { StyleSheet, Text, View, Image, FlatList, ScrollView, SafeAreaView, TouchableOpacity, Modal, Platform } from 'react-native';
...
export default function Lanches({ navigation }) {
  ...
  const dataCollectionRef = collection(db, "lanches")
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setLanches((data.docs.map((doc, item) => ({ ...doc.data(), id: doc.id, image: images[item] }))));
    };
    getData();
  }, [])
