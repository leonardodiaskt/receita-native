...
import { StyleSheet, Text, View, Image, FlatList, Modal, ScrollView, Platform, SafeAreaView, TouchableOpacity } from 'react-native';
...
export default function Massas() {
  ...
  const dataCollectionRef = collection(db, "massas")
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setMassas((data.docs.map((doc, item) => ({ ...doc.data(), id: doc.id, image: images[item] }))));
    };
    getData();
  }, [])
