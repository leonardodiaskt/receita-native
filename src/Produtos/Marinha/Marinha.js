...
import { StyleSheet, Text, View, Modal, Image, FlatList, ScrollView, SafeAreaView, TouchableOpacity, Platform } from 'react-native';
...
export default function Marinha({ navigation }) {

  const [nome, setNome] = useState("");
  const [descricao, setDescricao] = useState("");
  const [marinha, setMarinha] = useState("");
  const [visivel, setVisivel] = useState(false);
  const [visible, setVisible] = useState(false);
  const [url, setUrl] = useState("")

  const dataCollectionRef = collection(db, "marinha")
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setMarinha((data.docs.map((doc, item) => ({ ...doc.data(), id: doc.id, image: images[item] }))));
    };
    getData();
  }, [])
