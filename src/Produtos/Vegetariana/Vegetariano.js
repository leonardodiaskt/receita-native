...
import { StyleSheet, Text, View, Image, FlatList, ScrollView, Platform, TouchableOpacity, Modal } from 'react-native';
...
export default function Vegetariano() {

  const [nome, setNome] = useState("");
  const [descricao, setDescricao] = useState("");
  const [vegatariana, setVegetariana] = useState("");
  const [visivel, setVisivel] = useState(false);
  const [visible, setVisible] = useState(false);
  const [url, setUrl] = useState("")

  const dataCollectionRef = collection(db, "vegetariano")
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setVegetariana((data.docs.map((doc, item) => ({ ...doc.data(), id: doc.id, image: images[item] }))));
    };
    getData();
  }, [])
